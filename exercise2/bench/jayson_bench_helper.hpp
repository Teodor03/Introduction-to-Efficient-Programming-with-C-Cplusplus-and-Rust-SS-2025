...
inline std::string generate_random_strings(size_t count, size_t avg_length = 20) {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        ss << '"';
        for (size_t j = 0; j < length; ++j) { ss << static_cast<char>(char_dist(gen)); }
        ss << '"';
        if (i < count - 1) ss << ' ';
    }
    return ss.str();
...
inline std::string generate_random_key(std::mt19937 &gen, size_t avg_length = 10) {
    ...
    size_t length   = len_dist(gen);
    std::string key = "\"";
    for (size_t j = 0; j < length; ++j) { key += static_cast<char>(char_dist(gen)); }
    key += "\"";
    return key;
}
...
inline std::string generate_random_object(size_t num_keys) {
    ...
    for (size_t i = 0; i < num_keys; ++i) {
        ...
        case 2: { // String
            ...
            ss << "\"";
            for (size_t j = 0; j < str_len; ++j) { ss << static_cast<char>(char_dist(gen)); }
            ss << "\"";
            break;
        }
        case 3: // Boolean
...
inline std::string generate_complex_json(size_t depth = 3, size_t width = 5) {
    if (depth == 0) {
        ...
        case 2: { // String
            size_t str_len     = str_len_dist(gen);
            std::string result = "\"";
            for (size_t j = 0; j < str_len; ++j) { result += static_cast<char>(char_dist(gen)); }
            result += "\"";
            return result;
        }
        case 3: // Boolean
...
inline std::string generate_long_string(size_t length) {
    ...
    for (size_t i = 0; i < length; ++i) { result += static_cast<char>(char_dist(gen)); }
